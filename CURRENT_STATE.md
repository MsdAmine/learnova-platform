...
- Certificate PDF download: `GET /api/v1/learner/certificates/{certificateId}/pdf` (`CertificatePdfService`) generates a branded PDF on demand from trusted certificate/course/learner data (learner name, course title, instructor name, issue date, certificate code, Learnova/issued-by footer) and streams it as `application/pdf` with an attachment `Content-Disposition`; the PDF is never stored — no certificate media storage exists. Self-scoped (own certificate only, `404` otherwise). No QR code, digital signature, revocation, or public verification endpoint exists.
...
- Media uploads: new `media` module (`MediaFolder`, `MediaUploadResult`, `MediaStorageService` interface, `MediaValidator`, `CloudinaryConfig`, `CloudinaryMediaStorageService`) — backend-authenticated upload to Cloudinary; credentials are backend-only and never exposed to the frontend; no direct/unsigned frontend-to-Cloudinary upload exists. Live upload against real Cloudinary credentials (cloud `dnd5pu5me`) has been verified end-to-end for both the learner profile image and the instructor course thumbnail, including replacement uploads (deterministic public-ID overwrite, so the explicit-delete branch only fires when the public ID itself changes); Cloudinary dashboard verification (the web console, not the rendered URL) was not performed
  - `POST /api/v1/learner-profile/me/image` — learner uploads their own profile image (multipart field `file`); self-resolved from the authenticated principal, same pattern as other profile self-edit endpoints; `LearnerProfile.profileImagePublicId` stores the Cloudinary public id
  - `POST /api/v1/instructor/courses/{courseId}/thumbnail` — instructor uploads a thumbnail for their own course (multipart field `file`); INSTRUCTOR role + ownership-checked (`403` for another instructor's course); `Course.thumbnailPublicId` stores the Cloudinary public id
  - `MediaValidator` rejects empty files, non-`image/jpeg`/`image/png`/`image/webp` content types, and oversized files with `400 Bad Request` before any Cloudinary call is made
  - On replacement upload, the previous Cloudinary public id (if present) is deleted; deletion failures are logged and non-fatal since the new upload has already succeeded
  - Instructor profile image upload does not exist (`InstructorProfile` has no image field); lesson attachments and certificate media/PDF storage are not implemented and were not touched by this feature
...
A React + TypeScript + Vite frontend exists and is actively in development. It now has a minimal automated test harness (Vitest + React Testing Library + jsdom): 39 tests, run via `npm run test`. (Backend: 241 tests via `./mvnw test`, including the live-Cloudinary-credentials configuration hardening and the learner onboarding/learning-preferences integration tests — Cloudinary itself remains mocked in the automated suite.) Coverage is selective — `useProfileSwitch` success/failure, the learner dashboard's certificate-section states, the `learnerQuizzes` API client's attempt-history contract, the `CoursePlayer` quiz history UI extracted into `QuizCard`/`AttemptHistory` (`frontend/src/features/dashboard/components/courseQuiz/QuizCard.tsx`), the `liveSessions` API client / `LiveSessionsPage` UI, and the new Cloudinary media-upload integration (`src/api/profile.ts` and `src/api/instructorCourses.ts` multipart upload functions, `SettingsPage` photo uploader, `InstructorCoursesPage` thumbnail uploader). `frontend/src/test/setup.ts` now runs `afterEach(cleanup)` to prevent React Testing Library DOM leakage between tests. There is no broad frontend integration suite yet, and the full `CoursePlayer` route-level flow (tab switching, data fetching, lesson tab, certificate panel) remains untested at that level. Browser QA remains important for visual/accessibility checks.
...
- Learner dashboard and My Courses wired to real enrollment data; the dashboard's Certificates section reads real certificates via `getMyCertificates()` (loading skeleton, accessible error state with retry, real empty state) and each card links to `/dashboard/certificates/:certificateId`; the dashboard's previous local-placeholder "Upcoming Live Sessions" section and its earlier fake "Download" certificate action have been removed — `CertificateViewPage` and `CertificatesPage` now have a real "Download PDF" action calling `GET /api/v1/learner/certificates/{certificateId}/pdf` (`src/api/certificates.ts` → `downloadCertificatePdf()`), alongside the existing "Print certificate" (`window.print()`) action
...
- Live sessions: `LiveSessionsPage` (`/dashboard/live-sessions`) — learner view; lists upcoming sessions for enrolled courses via `GET /api/v1/learner/live-sessions/upcoming` and joins via `POST /api/v1/learner/live-sessions/{sessionId}/join`, opening the returned Jitsi URL in a new browser tab (no iframe embed). `InstructorLiveSessionsPage` (`/instructor/live-sessions`, under `InstructorLayout`) — instructor view; schedules sessions (modal form with course select, title, optional description, start/end time, optional max participants), lists own sessions across courses, and cancels a `SCHEDULED` session with inline confirm. `InstructorLayout`'s nav includes a "Live sessions" tab; the desktop nav row (`hidden md:flex`) is supplemented by a dedicated mobile-only nav row (`md:hidden`) with the same Courses and Live sessions links, so `/instructor/live-sessions` is reachable from in-app navigation at every viewport. Uses `src/api/liveSessions.ts`. Old fake `meet.learnova.app` / `recordings.learnova.app` placeholder links have been removed.
- API clients implemented: `src/api/courseContent.ts` (learner content + lesson progress), `src/api/instructorCourseContent.ts` (section/lesson CRUD), `src/api/adminInstructorProfiles.ts` (pending list, approve, reject), `src/api/instructorCourses.ts` (course CRUD/publish/archive, plus multipart course-thumbnail upload), `src/api/categories.ts` (category listing used in instructor course form), `src/api/instructorQuizzes.ts` (quiz/question/option CRUD, publish, archive, list, detail; exports `QuizResponse`, `QuizDetailResponse`, `QuestionResponse`, `AnswerOptionResponse`, `QuizStatus`, `QuestionType`, and all request payload types), `src/api/learnerQuizzes.ts` (learner quiz list, quiz detail, start/resume attempt, submit attempt, get attempt result; exports `LearnerQuizSummaryResponse`, `LearnerQuizDetailResponse`, `QuizAttemptResponse`, `QuizAttemptAnswerResultResponse`, `QuizAttemptSubmitRequest`, `QuizAttemptStatus`, `QuestionType`), `src/api/profile.ts` (get/update learner profile, update instructor profile, plus multipart learner profile-image upload)
- Media uploads: `SettingsPage` lets a learner upload a profile photo (`POST /api/v1/learner-profile/me/image`) with preview, loading, and accessible error states; `InstructorCoursesPage` lets an instructor upload a course thumbnail (`POST /api/v1/instructor/courses/{courseId}/thumbnail`) in EDIT mode only — create mode remains URL-only because no `courseId` exists yet before the course is created. `src/api/axios.ts` no longer hard-codes a `Content-Type: application/json` default, so these multipart requests work through the shared Axios instance. There is no instructor profile image upload (no UI, no backend field), no lesson-attachment upload, and no direct/unsigned browser-to-Cloudinary upload anywhere in the app. Live QA against real Cloudinary credentials confirmed both uploaders end-to-end: preview updates immediately, the uploaded image persists after reload, and the public catalog/course-detail page renders the new course thumbnail.
...
- Certificate PDF download exists (`GET /api/v1/learner/certificates/{certificateId}/pdf`, generated on demand, not stored); browser print/save-as-PDF (`window.print()`) is still available alongside it. No QR code, digital signature, revocation, or public verification endpoint.
- Live sessions v1 limitations: no `/leave` endpoint; no recurring sessions; no reminders; no past-session history view; the Jitsi room is opened in a new browser tab (no iframe embedding, no Jitsi JWT/JaaS); the public `meet.jit.si` room has no Learnova-side auth after the URL is handed to the learner — an unguessable room name is the v1 security boundary
...
- No instructor profile image upload (`InstructorProfile` has no image URL field, so there is no model to upload into yet)
- Cloudinary dashboard verification (manually confirming the uploaded assets in the Cloudinary web console) has not been performed; live upload success was confirmed via rendered URLs and persistence checks only
